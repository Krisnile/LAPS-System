...
- **基于提示的分割标注**：SAM 权重在 **`model/sam/`**，YOLO 分割权重在 **`model/yolo/`**；标注页可选模型，推理见 `apps/pages/sam_inference.py` 与 **`/segment-image/`**（`model=sam|yolo`）。
...
- **数据集**：列表页 `/datasets/` 仅展示基本信息（名称、图片数、创建时间、描述等）与 **新建导入**；每条可 **管理** 进入 `/datasets/<id>/` 详情页，在详情页完成 **继续导入**、缩略图 **预览与备注**、单图删除及数据集元数据编辑。创建时可选择导入类型：本地多图 / ZIP / URL 列表（每行一个 HTTP(S) 地址，可用「 | 」或 Tab 在地址后写备注，写入图片 `caption`）；详情页图片列表分页加载（API：`/api/datasets/<id>/images/`），首屏一页默认可配。项目与数据集的关联在 **项目** 页维护，数据集页不展示关联项目列。
